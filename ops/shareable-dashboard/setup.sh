...
# Usage:
#   sudo DOMAIN=dashboard.example.com ADMIN_USER=perez ADMIN_PASSWORD='strong-password' bash ops/shareable-dashboard/setup.sh
#
...
if [[ -z "$DOMAIN" || "$DOMAIN" == *" "* ]]; then echo "ERROR: set DOMAIN to your real DNS hostname, e.g. DOMAIN=dashboard.example.com"; exit 2; fi
if [[ -z "$ADMIN_PASSWORD" || ${#ADMIN_PASSWORD} -lt 14 ]]; then echo "ERROR: ADMIN_PASSWORD must be at least 14 characters."; exit 3; fi
...
[[ "$HTTP_CODE" == "401" || "$HTTP_CODE" == "400" ]] || { echo "Unexpected protected endpoint status: $HTTP_CODE"; exit 20; }
...
printf 'URL: https://%s\n' "$DOMAIN"
printf 'USER: %s\n' "$ADMIN_USER"
printf 'Password: the value supplied to ADMIN_PASSWORD\n'
