...
DB=ROOT/"data/memory/dashboard_telemetry.sqlite3"
IST=ZoneInfo("Asia/Kolkata")
def _now(): return datetime.now(IST).isoformat(timespec="milliseconds")
def _db():
    DB.parent.mkdir(parents=True,exist_ok=True); c=sqlite3.connect(DB,timeout=3); c.execute("PRAGMA journal_mode=WAL")
    c.execute("CREATE TABLE IF NOT EXISTS lifecycle(id INTEGER PRIMARY KEY AUTOINCREMENT,event_key TEXT NOT NULL,stage TEXT NOT NULL,status TEXT NOT NULL,ts TEXT NOT NULL,symbol TEXT,option_type TEXT,contract TEXT,score REAL,details_json TEXT NOT NULL DEFAULT '{}')")
    c.execute("CREATE INDEX IF NOT EXISTS idx_lifecycle_event ON lifecycle(event_key,id)"); c.execute("CREATE INDEX IF NOT EXISTS idx_lifecycle_ts ON lifecycle(ts)"); return c
def record_stage(event_key:str,stage:str,status:str="OK",**details:Any)->None:
    if not event_key:return
    try:
        with _db() as db: db.execute("INSERT INTO lifecycle(event_key,stage,status,ts,symbol,option_type,contract,score,details_json) VALUES (?,?,?,?,?,?,?,?,?)",(str(event_key),str(stage),str(status),_now(),details.pop("symbol",None),details.pop("option_type",None),details.pop("contract",None),details.pop("score",None),json.dumps(details,default=str,separators=(",",":"))))
