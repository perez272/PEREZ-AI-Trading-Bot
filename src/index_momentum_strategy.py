...
def _signal_score(candidate: dict[str, Any]) -> tuple[int, list[str]]:
    ...
    base = min(60, max(0, int(_num(candidate, "score"))))
    score = base
    reasons: list[str] = ["BASE_SCANNER_CONFIRMATION"]
    ...
    if m15 == directional and h1 == directional:
        score += 15; reasons.append("MTF_DIRECTION")
    if volume_ratio >= 1.5:
        score += 12; reasons.append("VOLUME_EXPANSION")
    elif volume_ratio >= 1.2:
        score += 7; reasons.append("VOLUME_CONFIRMATION")

    # RSI is used as a momentum/overextension filter, not as a standalone signal.
    if signal == "BUY CE" and 52 <= rsi <= 72:
        score += 8; reasons.append("BULLISH_RSI_ZONE")
    elif signal == "BUY PE" and 28 <= rsi <= 48:
        score += 8; reasons.append("BEARISH_RSI_ZONE")
    elif signal == "BUY CE" and rsi > 78:
        score -= 18; reasons.append("OVEREXTENDED_RSI")
    elif signal == "BUY PE" and rsi < 22:
        score -= 18; reasons.append("OVEREXTENDED_RSI")

    # Optional enhanced scanner fields are used when available; older scanner
    # output remains valid but cannot earn these extra points.
    ...
    if breakout >= 1.0:
        score += 10; reasons.append("BREAKOUT")
    elif breakout >= 0.6:
        score += 5; reasons.append("EARLY_BREAKOUT")
    if body >= 0.60:
        score += 5; reasons.append("STRONG_CANDLE_BODY")
    if ema_gap >= 0.10:
        score += 5; reasons.append("EMA_SEPARATION")
    if (signal == "BUY CE" and rsi_slope > 0) or (signal == "BUY PE" and rsi_slope < 0):
        score += 5; reasons.append("MOMENTUM_SLOPE")
    if 0 < atr_pct < 0.08:
        score -= 8; reasons.append("LOW_REALIZED_RANGE")
...
def select_index_momentum_candidate(results: Iterable[dict[str, Any]], minimum_score: int = MIN_SCORE) -> dict[str, Any] | None:
    ...
    candidates = []
    for raw in results:
        if str(raw.get("symbol", "")).upper() not in INDEX_SYMBOLS:
            continue
        momentum_score, reasons = _signal_score(raw)
        candidate = dict(raw)
    ...
    return max(candidates, key=lambda item: (item["momentum_score"], item.get("score", 0))) if candidates else None


def build_dynamic_exits(entry: float, atr: float, option_ltp: float) -> dict[str, float]:
    """Build bounded option-premium exits from underlying volatility."""
    entry = float(entry)
