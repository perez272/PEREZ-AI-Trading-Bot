...
from src.ai_scoring import calculate_score, calculate_directional_score
...
def _get_index_futures_participation(symbol):
    """Get real traded participation from the nearest index future.

    Spot index volume can legitimately be zero because the index itself
    is not a traded instrument. Use the corresponding index future for
    participation and cache the result for the current 5-minute bucket.
    """
    index_symbols = {
        "NIFTY",
        "BANKNIFTY",
    ...
    if symbol not in index_symbols:
        return {
            ...
            "volume_source": "native",
            "participation_ratio": 0.0,
            "participation_source": "native",
        }

    bucket = _closed_candle_bucket(datetime.now(IST))
    cache = getattr(_get_index_futures_participation, "_cache", {})
    ...
    try:
        router = _get_router()
        candles, metadata, source = router.get_index_futures_candles(
            symbol,
        ...
        if not candles or len(candles) < 21:
            fields = {
                "volume_ratio": 0.0,
                "volume_available": False,
                "volume_source": "index_future_unavailable",
                "participation_ratio": 0.0,
                "participation_source": "index_future_unavailable",
                "futures_instrument_key": metadata.get("instrument_key"),
                "futures_expiry": metadata.get("expiry"),
                "futures_trading_symbol": metadata.get("trading_symbol"),
            }
        else:
            volumes = []
            ...
            if len(volumes) < 21:
                fields = {
                    "volume_ratio": 0.0,
                    "volume_available": False,
                    "volume_source": "index_future_invalid",
                    "participation_ratio": 0.0,
                    "participation_source": "index_future_invalid",
                }
            else:
                ...
                fields = {
                    ...
                    "volume_source": "upstox_index_future",
                    "participation_ratio": float(ratio),
                    "participation_source": "upstox_index_future",
                    "futures_instrument_key": metadata.get("instrument_key"),
                    "futures_expiry": metadata.get("expiry"),
                    "futures_trading_symbol": metadata.get("trading_symbol"),
                }
        ...
        _get_index_futures_participation._cache = cache

        return fields

    except Exception as exc:
        print(
            f"[MARKET DATA] Futures participation failed for {symbol}: {exc}"
        )

        fields = {
            "volume_ratio": 0.0,
            "volume_available": False,
            "volume_source": "index_future_error",
            "participation_ratio": 0.0,
            "participation_source": "index_future_error",
        }
...
def _scan_one(symbol, exchange, token):
    ...
    try:
        ...
        # Preserve the legacy score for the CE/PE decision itself.
        # Once direction is known, score the market in that direction.
        directional_score = base_score
        if signal == "BUY CE":
            directional_score = calculate_directional_score(df, "BULLISH")
        elif signal == "BUY PE":
            directional_score = calculate_directional_score(df, "BEARISH")

        mtf = confirm_multi_timeframe(candles)
        score = max(0, min(100, int(directional_score) + int(mtf["quality"])))
        fields = _derive_momentum_fields(df)

        # Preserve the strategy decision separately from the final admitted
        # signal.  MTF remains a hard safety gate; this only makes the
        # rejection reason observable instead of turning a valid BUY CE/PE
        # into an ambiguous INVALID_SIGNAL.
        raw_signal = signal
        signal_block_reason = None
        if raw_signal in ("BUY CE", "BUY PE") and not mtf["aligned"]:
            signal_block_reason = "MTF_NOT_ALIGNED"
            signal = "NO TRADE"

        # Index spot candles are price data; their native volume can be
        # zero. Use the nearest traded index future for participation.
        if symbol.upper() in {
            "NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "SENSEX"
        }:
            fields.update(_get_index_futures_participation(symbol))
        candidate = {
