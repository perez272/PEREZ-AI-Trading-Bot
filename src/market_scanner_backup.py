"""Legacy market scanner kept for compatibility.

Importing the module is side-effect free. Broker authentication and market
requests happen only when ``scan_market`` is explicitly invoked.
"""
...
def scan_market():
    """Scan the legacy symbol set when explicitly requested."""
    session = SessionManager(API_KEY, CLIENT_ID, PASSWORD, TOTP_SECRET)
    client = AngelClient(session.get_client())
    results = []
    ...
    for symbol, (exchange, token) in SYMBOLS.items():
        ...
        try:
            ...
            signal, trend = get_trade_decision(
                score, float(last["RSI"]), float(last["EMA20"]),
                float(last["EMA50"]), float(last["close"]),
            )
            results.append({
                "symbol": symbol, "score": score, "close": float(last["close"]),
...
def print_results(results):
    ...
    for item in results:
        print(
            ...
            f"Close={item['close']:.2f} Signal={item['signal']}"
        )
    print("-" * 60)
