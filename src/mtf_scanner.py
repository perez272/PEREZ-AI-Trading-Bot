"""PRO scanner: parallel 5m/15m/60m data with strict admission gates."""
...
def _scan_one(symbol, exchange, token):
    try:
        with ThreadPoolExecutor(max_workers=3) as pool:
            futures = {tf: pool.submit(_fetch, symbol, exchange, token, tf) for tf in TIMEFRAMES}
            frames = {tf: futures[tf].result() for tf in TIMEFRAMES}
