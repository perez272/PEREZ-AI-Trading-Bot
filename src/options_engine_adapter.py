...
def enrich_with_live_option_data(candidate: Dict[str, Any]) -> Dict[str, Any]:
    """Replace option evidence with one fresh broker FULL quote; fail closed."""
    result = dict(candidate)
    for key in ("ltp", "volume", "open_interest", "best_bid", "best_ask", "spread_pct",
                "slippage_pct", "buy_quantity", "sell_quantity", "last_trade_qty", "avg_price",
                "net_change", "percent_change", "volume_score", "oi_score", "oi_change_score",
                "iv_score", "liquidity_score"):
        result.pop(key, None)
    ...
    try:
        ...
        # Long-entry slippage is conservatively measured from LTP to best ask.
        # If either side of the book is missing, fail the slippage gate instead of fabricating zero.
        if bid > 0 and ask > 0:
        ...
        # No genuine prior-OI snapshot exists here, so OI-change remains zero.
