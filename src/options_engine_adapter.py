...
def _get_live_option_client():
    ...
    if _live_option_client is not None: return _live_option_client
...
def evidence_from_candidate(candidate: Dict[str, Any]) -> OptionEvidence:
    return OptionEvidence(
        ...
        trend_score=_num(candidate.get("trend_score")), momentum_score=_num(candidate.get("momentum_score")),
        volume_score=_num(candidate.get("volume_score")), vwap_score=_num(candidate.get("vwap_score")),
        volatility_score=_num(candidate.get("volatility_score")), structure_score=_num(candidate.get("structure_score")),
        oi_score=_num(candidate.get("oi_score")), oi_change_score=_num(candidate.get("oi_change_score")),
        iv_score=_num(candidate.get("iv_score")), liquidity_score=_num(candidate.get("liquidity_score")),
...
def enrich_with_live_option_data(candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in ("ltp", "volume", "open_interest", "best_bid", "best_ask", "spread_pct", "slippage_pct",
        ...
        result.pop(key, None)
    result["live_market_data"] = False; result["live_data_error"] = "NOT_ATTEMPTED"
    ...
    if not token:
        result["live_data_error"] = "MISSING_OPTION_TOKEN"; return result
    try:
        ...
        if not fetched: result["live_data_error"] = "NO_MARKET_DATA"; return result
        quote = fetched[0]; ltp = _num(quote.get("ltp"))
        if ltp <= 0: result["live_data_error"] = "INVALID_LTP"; return result
        ...
        bid = _num(buys[0].get("price")) if buys else 0.0; ask = _num(sells[0].get("price")) if sells else 0.0
        ...
        result["oi_change_score"] = 0.0; result["oi_change_available"] = False
        result["iv_score"] = 0.0; result["iv_available"] = False; result["greeks_available"] = False
        ...
        else: result["liquidity_score"] = 0.0
        result["live_market_data"] = True; result["live_data_error"] = ""
        return result
    except Exception as exc:
        result["live_data_error"] = repr(exc); return result
...
def evaluate_candidate(candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not enriched.get("live_market_data"):
        enriched["options_gate"] = {"score": 0, "eligible": False, "decision": "NO TRADE", "reasons": ["LIVE_OPTION_DATA_UNAVAILABLE"], "levels": {}, "live_orders": False, "paper_trade": True}
        enriched["options_score"] = 0; enriched["paper_trade_candidate"] = False; enriched["live_orders"] = False; return enriched
    gate = validate_trade(evidence_from_candidate(enriched))
    enriched["options_gate"] = gate; enriched["options_score"] = gate["score"]; enriched["paper_trade_candidate"] = gate["eligible"]; enriched["live_orders"] = False
    return enriched

def evaluate_option_candidate(candidate): return evaluate_candidate(candidate)
def evaluate_candidates(candidates: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]: return [evaluate_candidate(c) for c in candidates]
...
def print_gate_summary(result: Dict[str, Any]) -> None:
    gate = result.get("options_gate", {}); print("-" * 72); print(f"{result.get('symbol', '')} {result.get('option_type', '')}"); print(f"OPTIONS SCORE : {gate.get('score', 0)}/100"); print(f"DECISION      : {gate.get('decision', 'NO TRADE')}")
    if gate.get("reasons"): print("REJECT REASONS:", ", ".join(gate["reasons"]))
    print("PAPER TRADE   :", True); print("LIVE ORDERS   :", False)
