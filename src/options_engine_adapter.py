...
def evidence_from_candidate(candidate: Dict[str, Any]) -> OptionEvidence:
    return OptionEvidence(
        ...
        trend_score=_num(candidate.get("trend_score") or candidate.get("trend")),
        momentum_score=_num(candidate.get("momentum_score") or candidate.get("momentum")),
        volume_score=_num(candidate.get("volume_score") or candidate.get("volume")),
        vwap_score=_num(candidate.get("vwap_score") or candidate.get("vwap")),
        volatility_score=_num(candidate.get("volatility_score") or candidate.get("volatility")),
        structure_score=_num(candidate.get("structure_score") or candidate.get("structure")),
        oi_score=_num(candidate.get("oi_score") or candidate.get("open_interest_score")),
        oi_change_score=_num(candidate.get("oi_change_score") or candidate.get("oi_change") or candidate.get("oi_change_pct")),
        iv_score=_num(candidate.get("iv_score") or candidate.get("implied_volatility_score")),
        liquidity_score=_num(candidate.get("liquidity_score") or candidate.get("liquidity")),
        index_confirmation=_num(candidate.get("index_confirmation") or candidate.get("index_score")),
        news_confirmation=_num(candidate.get("news_confirmation") or candidate.get("news_score")),
        event_risk_penalty=_num(candidate.get("event_risk_penalty") or candidate.get("event_penalty")),
        spread_pct=_num(candidate.get("spread_pct") or candidate.get("spread")),
        slippage_pct=_num(candidate.get("slippage_pct") or candidate.get("slippage")),
    )


def enrich_with_live_option_data(candidate: Dict[str, Any]) -> Dict[str, Any]:
    """Read-only Angel One FULL market-data enrichment. No order methods are called."""
    result = dict(candidate)

    for key in (
        "ltp", "volume", "open_interest", "best_bid", "best_ask", "spread_pct",
        "buy_quantity", "sell_quantity", "last_trade_qty", "avg_price", "net_change",
        "percent_change", "volume_score", "oi_score", "oi_change_score", "iv_score",
        "liquidity_score",
    ):
        result.pop(key, None)
    ...
    if isinstance(contract, dict):
        exchange = exchange or _text(contract.get("exchange") or contract.get("exch_seg"), "NFO")
        token = token or _text(contract.get("token") or contract.get("symbolToken") or contract.get("symbol_token"))
    ...
    try:
        ...
        response = client.get_market_data("FULL", {exchange: [token]})
        ...
        if ltp > 0 and best_bid > 0 and best_ask > 0:
            result["spread_pct"] = ((best_ask - best_bid) / ltp) * 100
        ...
        if result["volume"] > 0 and result["open_interest"] > 0:
            result["oi_volume_ratio"] = result["open_interest"] / result["volume"]

        if result["buy_quantity"] > result["sell_quantity"]:
            result["liquidity_score"] = min(7.0, 4.0 + (result["buy_quantity"] / max(result["sell_quantity"], 1) - 1.0))
        else:
...
def evaluate_candidate(candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not enriched.get("live_market_data"):
        enriched["options_gate"] = {
            "score": 0, "eligible": False, "decision": "NO TRADE",
    ...
    # FAST SURGE MODE: every normal option evaluation also records a live
    # snapshot and checks the 5/10/15-minute surge windows. A surge is evidence,
    # never a standalone BUY trigger.
    try:
        surge_events = observe_option(
            candidate,
            enriched,
        ...
        surge_events = []
        enriched["surge_error"] = repr(exc)

    enriched["surge_events"] = surge_events
    enriched["surge_detected"] = bool(surge_events)
    if surge_events:
        strongest = max(surge_events, key=lambda x: (x.get("change_pct", 0), x.get("window_minutes", 999)))
        enriched["surge_summary"] = {
            "change_pct": strongest.get("change_pct"),
            "window_minutes": strongest.get("window_minutes"),
            "threshold_pct": strongest.get("threshold_pct"),
            "expiry_bucket": strongest.get("expiry_bucket"),
            "start_ltp": strongest.get("start_ltp"),
            "end_ltp": strongest.get("end_ltp"),
        }
...
def evaluate_candidates(candidates: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for candidate in candidates:
        try:
            evidence.append(evidence_from_candidate(candidate))
...
def select_paper_trade(candidates: Iterable[Dict[str, Any]]):
    ...
    eligible = [x for x in evaluated if x.get("paper_trade_candidate") is True]
    if not eligible:
        return None
    eligible.sort(key=lambda x: x.get("options_score", 0), reverse=True)
    return eligible[0]
...
def print_gate_summary(result: Dict[str, Any]) -> None:
    ...
    surge = result.get("surge_summary")
    if surge:
        print(
            f"SURGE         : +{surge['change_pct']:.2f}% in {surge['window_minutes']}m "
            f"Rs {surge['start_ltp']:.2f} -> Rs {surge['end_ltp']:.2f} "
            f"({surge['expiry_bucket']})"
        )
