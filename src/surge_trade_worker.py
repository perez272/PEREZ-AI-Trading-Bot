...
def _process_once():
 ...
 except Exception as exc:write_heartbeat('surge_bridge_capital_error',error=str(exc));return False
 ...
 for event in events:
  ...
  # Learn only from information available before the decision. This is
  # deliberately separate from the existing risk/gate settings.
  try:
  ...
  # Memory is advisory telemetry here; existing gate/risk rules remain authoritative.
  if isinstance(gate,dict) and event.get('learning',{}).get('status')=='LEARNED':
   gate['learning_adjustment']=event['learning'].get('adjustment',0.0);gate['learning_confidence']=event['learning'].get('confidence',0.0)
  ...
  if trade is None:
   ...
   continue
  trade['strategy']='SURGE_EARLY_EXPLOSIVE';trade['surge_score']=gate.get('score',0);trade['surge_reasons']=gate.get('reasons',[]);trade['learning']=event.get('learning',{})
  ...
  try:
   resolve_outcome(event_key=event_key,candidate=event,pnl=result_monitor.get('pnl',0),pnl_percent=result_monitor.get('pnl_percent',0),mfe=result_monitor.get('mfe'),mae=result_monitor.get('mae'),exit_reason=result_monitor.get('exit_reason',''))
