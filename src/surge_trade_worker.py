...
def _process_once():
 ...
 try:
  client=get_client();capital=get_available_capital(client,paper_mode=True);allowed,_reason,_summary=can_open_new_trade(MAX_TRADES_PER_DAY,None,capital)
 ...
 for event in events:
  ...
  append_audit('SURGE_PAPER_TRADE_CREATED',f"{symbol} {option_type} score={trade['surge_score']}",event_key,'RECORDED');write_heartbeat('surge_paper_trade',symbol=symbol,contract=trade.get('contract'),strategy=trade['strategy']);print(f"[SURGE BRIDGE] PAPER TRADE {trade.get('contract')} score={trade['surge_score']} qty={trade.get('quantity')}")
