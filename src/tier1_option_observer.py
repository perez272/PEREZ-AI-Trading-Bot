...
from src.surge_outcome_learning import backfill_existing_surge_candidates,remember_surge,record_quote
...
class Tier1OptionObserver:
 ...
 def _init_db(self):
  with self._connect() as db:
   ...
   db.execute("""CREATE TABLE IF NOT EXISTS early_events(id INTEGER PRIMARY KEY AUTOINCREMENT,event_key TEXT UNIQUE NOT NULL,symbol TEXT NOT NULL,option_type TEXT NOT NULL,instrument_key TEXT NOT NULL,expiry TEXT,strike REAL,ltp REAL NOT NULL,score REAL NOT NULL,move_1m_pct REAL,move_3m_pct REAL,move_5m_pct REAL,velocity REAL,acceleration REAL,volume_ratio REAL,spread_pct REAL,reasons_json TEXT NOT NULL,features_json TEXT NOT NULL,observed_ts TEXT NOT NULL,detection_ts TEXT NOT NULL,consumed INTEGER NOT NULL DEFAULT 0)""")
   db.execute("CREATE INDEX IF NOT EXISTS idx_early_events_pending ON early_events(consumed,observed_ts)")
   db.execute("CREATE TABLE IF NOT EXISTS observer_meta(key TEXT PRIMARY KEY,value INTEGER NOT NULL)")
   existing=db.execute("SELECT value FROM observer_meta WHERE key='surge_events_total'").fetchone()
 ...
 def _contract_key(symbol,row,option_type):
  raw="|".join(str(x or "") for x in (symbol,option_type,row.get("instrument_key"),row.get("expiry"),row.get("strike_price")));return hashlib.sha256(raw.encode()).hexdigest()
 @staticmethod
 ...
 def _features(self,symbol,option_type,row,market,move_pct,baseline):
  md=market.get("market_data") or {};greeks=market.get("option_greeks") if isinstance(market.get("option_greeks"),dict) else {}
  return {"symbol":symbol,"option_type":option_type,"instrument_key":market.get("instrument_key"),"contract":market.get("trading_symbol") or row.get("trading_symbol"),"expiry":row.get("expiry"),"strike":row.get("strike_price"),"move_pct":round(move_pct,4),"baseline_ltp":round(baseline,4),"ltp":md.get("ltp"),"bid":md.get("bid_price"),"ask":md.get("ask_price"),"volume":md.get("volume"),"oi":md.get("oi"),"iv":greeks.get("iv"),"delta":greeks.get("delta"),"gamma":greeks.get("gamma"),"theta":greeks.get("theta"),"vega":greeks.get("vega")}
 def _record_fast_signal(self,symbol,option_type,market,observed_ts):
  key=str(market.get("instrument_key") or "")
  if not key:return None
  snapshot=dict(market);snapshot["observed_ts"]=observed_ts
  try:record_quote(key,observed_ts,float((market.get("market_data") or {}).get("ltp") or 0))
  except Exception as exc:print(f"[SURGE LEARNING] quote update skipped: {exc}")
  signal=detect_explosive_move(symbol,option_type,snapshot,list(self._history[key]));self._history[key].append(snapshot);return signal
  ...
  with self._connect() as db:
   for row in chain or []:
    for option_type in ("CE","PE"):
     ...
     if not existing:
      db.execute("INSERT INTO baselines VALUES (?,?,?,?,?,?,?,?,?)",(key,symbol,option_type,row.get("expiry"),row.get("strike_price"),ltp,observed_ts,ltp,observed_ts));continue
     ...
     if baseline_age>BASELINE_TTL_SECONDS or ltp<baseline*0.5:
      db.execute("UPDATE baselines SET baseline_ltp=?,baseline_ts=?,last_ltp=?,last_ts=? WHERE contract_key=?",(ltp,observed_ts,ltp,observed_ts,key));continue
     ...
     for threshold in MOVE_THRESHOLDS:
      ...
      try:
       detection_ts=datetime.now(timezone.utc).isoformat();db.execute("INSERT INTO move_events(event_key,symbol,option_type,contract,expiry,strike,threshold,baseline_ltp,ltp,move_pct,observed_ts,detection_ts,features_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",(event_key,symbol,option_type,features.get("contract"),row.get("expiry"),row.get("strike_price"),threshold,baseline,ltp,move_pct,observed_ts,detection_ts,json.dumps(features,separators=(",",":"))));db.execute("UPDATE observer_meta SET value=value+1 WHERE key='surge_events_total'");events.append({"type":"THRESHOLD","threshold":threshold,**features})
      except sqlite3.IntegrityError:pass
  ...
  for symbol in TIER1_SYMBOLS:
   try:
    ...
    if chain and source=="upstox":
     observed_ts=datetime.now(timezone.utc).isoformat();events.extend(self.observe(symbol,chain,observed_ts=observed_ts));print(f"[TIER1 OBSERVER] {symbol}: observation source={source}")
